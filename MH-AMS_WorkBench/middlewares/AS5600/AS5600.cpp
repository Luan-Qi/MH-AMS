...
uint8_t AS5600::readReg(uint8_t reg)
{
  _error = AS5600_OK;
	...
	AS5600_Read8(_address, reg, 1, &_data);
...
uint16_t AS5600::readReg2(uint8_t reg)
{
  ...
  uint16_t _data = 0;
	...
	AS5600_Read8(_address, reg, 1, &data);
  _data += data;
	_data <<= 8;
  AS5600_Read8(_address, reg-1, 1, &data);
	_data += data;
  return _data;
}
...
uint8_t AS5600::writeReg(uint8_t reg, uint8_t value)
{
  _error = AS5600_OK;
	...
	AS5600_Write8(_address, reg, 1, &_value);
  return _error;
}
...
uint8_t AS5600::writeReg2(uint8_t reg, uint16_t value)
{
  _error = AS5600_OK;
	...
	AS5600_Write8(_address, reg, 2, _value);
  return _error;
}
